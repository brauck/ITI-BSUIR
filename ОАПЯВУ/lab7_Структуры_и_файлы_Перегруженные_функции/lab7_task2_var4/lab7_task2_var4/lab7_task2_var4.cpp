...
bool filenameInput(char* filename, int maxFileNameLength)
{
	char ch{};
	int i{};
	for (; i < maxFileNameLength - strlen(extension); i++)
	{
		std::cin.get(ch);
		if (ch == '\n')
		{
			filename[i] = '\0';
			strcat_s(filename, maxFileNameLength, extension);
			return true;
		}
		if (ch == '.')
		{
